...
class Sonic_GML():
    ...
    prev_notes = []
    note_num = 0
    pos_mid = screen_get_mid_position()
    note_pos = []
    orbit_pos = []
    notes_on_pos = []
    chord_size = 64
    ...
    def build_note_seq_nested(self, parent_node):
        ...
        if(parent_node.oscillators() > 10000):
            return
        self.notes.append(parent_node.distanceTo(self.pos_mid))
        [px, py] = parent_node.get_cartesian_pos()
        self.note_pos.append([px, py])
        self.note_num += 1
            ...
            for pos in self.orbit_pos:
                ...
                if (pos[2] == True):
                    if(self.green_dot == None):
                        self.green_dot = create_circular_blit(pos1, self.green_dot_size, 0, [
                                                              70, 255, 70], [0, 0, 0], 100, False, 0)
                    else:
                        plot_centre_blit(self.green_dot, pos1)
                else:
                    if(self.blue_dot == None):
                        self.blue_dot = create_circular_blit(pos1, self.blue_dot_size, 0, [
                                                             70, 70, 255], [0, 0, 0], 100, False, 0)
                    else:
                        plot_centre_blit(self.blue_dot, pos1)
        for [p1, p2, v] in self.notes_on_pos:
            ...
            if(self.draw_mode >= 1):
                if(self.blip_vol[bing_vol] == None):
                    self.blip_vol[bing_vol] = create_circular_blit(
                        pos, r, 0, [240, 0, bing_vol], [0, 0, 0], 190, False, 0)
                else:
                    plot_centre_blit(self.blip_vol[bing_vol], pos)
                if(self.blip == None):
                    self.blip = create_circular_blit(
                        pos, 1, 0, [240, 240, 240], [0, 0, 0], 190, False, 0)
                else:
                    plot_centre_blit(self.blip, pos)
            if(v < 12):
                if(self.draw_mode >= 1):
                    #Draw star
                    if(self.star == None):
                        self.star = create_star_blit(
                            pos, r*3, 4, [255, 0, 0], [0, 0, 0], 190)
                    else:
                        plot_centre_blit(self.star, pos)
    ...
    def play_sonic_sequence(self, rootNode):
        """
    ...
    def drawText(self, text_draw, text_canvas, offset):
        """
        Draw text related to the options selected
        """
        text_draw.drawText(text_canvas, 10, offset,
                           "Chord Size: " + str(int(self.chord_size)), 18)
        text_draw.drawText(text_canvas, 10, offset+20,
                           "Octaves: " + str(int(self.octaves)), 18)
        if(self.log_music_mode == True):
            text_draw.drawText(text_canvas, 10, offset+40, "Music: log", 18)
        else:
            text_draw.drawText(text_canvas, 10, offset+40, "Music: lin", 18)
        text_draw.drawText(text_canvas, 10, offset+60,
                           "Scale: " + str(int(self.music_scale*10)/10), 18)
        text_draw.drawText(text_canvas, 10, offset+80,
                           "GML: " + str_modes[self.gml_mode], 18)
        text_draw.drawText(text_canvas, 10, offset+100,
